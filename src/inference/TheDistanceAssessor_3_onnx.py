"""ONNX Runtime 기반 통합 추론 파이프라인.

TensorRT 버전과 동일한 레일/위험도 로직을 공유하지만, 모델 실행은 ONNX Runtime provider에
의존한다. provider 선택, 출력 텐서 shape, NMS 구현이 핵심 차이점이다.
"""
...
class ONNXSegmentationEngine:
    ...
    def infer(self, input_data):
        """전처리된 입력 배열을 ONNX 세션에 넣고 첫 번째 출력 logits를 반환한다."""

        result = self.session.run(self.output_names, {self.input_name: input_data})
        return result[0]  # 첫 번째 출력 반환

class ONNXYOLOEngine:
    """ONNX Runtime YOLO 추론과 후처리를 담당한다."""
    ...
    def predict(self, image):
        """원본 이미지를 모델 입력 크기에 맞춘 뒤 ONNX 추론을 수행한다."""
    ...
    def post_process(self, output, original_shape, input_size):
        """YOLO ONNX 출력을 원본 영상 좌표계의 박스 목록으로 복원한다."""

        # YOLO 출력 후처리
        # output shape: [1, 25200, 85] (YOLO v8 기준)
        if len(output.shape) == 3:
            output = output[0]  # batch dimension 제거
        ...
        for detection in output:
            ...
            if confidence > self.conf_threshold:
                if len(detection) > 5:  # class scores available
                    ...
                    # No class scores, use default class 0
                    x_center *= scale_x
                    y_center *= scale_y
                    width *= scale_x
                    height *= scale_y
    ...
    def apply_nms(self, boxes, scores, class_ids):
        ...
        while len(indices) > 0:
            ...
            indices = remaining_indices[iou <= self.iou_threshold]
        
        return boxes[keep].tolist(), scores[keep].tolist(), class_ids[keep].tolist()

def load_segmentation_engine(onnx_path):
    return ONNXSegmentationEngine(onnx_path)

def load_yolo_engine(onnx_path):
    return ONNXYOLOEngine(onnx_path)
...
def find_edges(image, y_levels, values=[4, 9], min_width=19):
    ...
    ONNX 백엔드이지만 레일 후보 탐색 기준과 휴리스틱은 TensorRT 버전과 동일하다.
    """
    edges_dict = {}
    for y in y_levels:
        row = image[y, :]
...
def segment(model_seg, image_size, filename, PATH_jpgs, dataset_type, model_type, item=None):
    ...
    # Convert to numpy and prepare for ONNX
...
def run(model_seg, model_det, image_size, filepath_img, PATH_jpgs, dataset_type, model_type, target_distances, file_index, vis, item=None, num_ys = 15):
    """단일 이미지 기준 ONNX 통합 추론 파이프라인을 실행한다."""
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please check if ONNX models are available and paths are correct")
        import traceback
        traceback.print_exc()
