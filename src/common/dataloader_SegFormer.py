"""SegFormer 학습 경로에서 사용하는 이미지 프로세서 연계 데이터로더.

이 데이터로더는 Albumentations 증강 이후 `SegformerImageProcessor`에 입력을
위임해 `pixel_values`와 `labels`를 생성한다. 따라서 일반 PyTorch 텐서 변환과
달리 Hugging Face SegFormer 학습 규격을 그대로 따르는 것이 특징이다.
"""
...
class CustomDataset(VisionDataset):
    """SegFormer용 전처리 규격을 반영한 이미지/마스크 데이터셋."""
    ...
    def __getitem__(self, idx):
        """증강된 샘플을 `SegformerImageProcessor` 규격으로 반환한다.

        `ignore=False`가 기본값이므로 원본 클래스 체계를 최대한 유지하고,
        라벨 `255`만 ignore index 성격의 `21`로 이동시킨다. 이 결정은
        SegFormer 전이학습 스크립트의 라벨 처리 방식과 맞물려 있다.
        """
