...
def image_morpho(mask_prediction):
    """작은 구멍과 끊김을 줄이기 위해 간단한 closing 연산을 적용한다."""
...
def remap_mask(mask1, mask2): # not used
    ...
    과거 클래스 인덱스 체계를 억지로 맞춰 비교하던 실험 흔적이다.
    현재 활성 평가 경로에서는 사용하지 않지만, 과거 결과 재현 가능성을 위해 남겨 둔다.
    """
...
def compute_map_cls(gt_mask, pred_mask, classes_ap, major = False, treshold=150):
    """이미지 단위 mAP와 클래스별 누적 AP를 계산한다.

    `major=True`이면 GT와 예측 모두에서 `treshold` 픽셀 이상 등장한 클래스만 평가한다.
    배경 클래스 `12`는 평균 계산에서 제외한다.
    """
...
def get_major_classes(gt_mask,pred_mask,treshold):
    """GT와 예측 모두에서 충분한 픽셀 수를 가진 클래스만 추린다."""
...
def compute_IoU(gt_mask, pred_mask, classes_stats, major=False, treshold=144):
    """IoU/정확도/정밀도/재현율과 클래스별 누적 통계를 계산한다.

    `major=True`면 `treshold` 픽셀 이상 등장한 클래스만 포함하며,
    배경 클래스 `12`는 제외한다.
    """
