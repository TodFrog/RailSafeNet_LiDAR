#!/usr/bin/env python3
"""최종 유지용 SegFormer 학습 진입 스크립트.

현재 저장소에서 active 상태로 남기는 SegFormer 계열 학습 엔트리이며, 과거 여러 실험 파일 중
`train_SegFormer_transfer_learning.py`를 canonical 이름으로 승격한 버전이다.

핵심 가정:
- 추론 파이프라인과 같은 13클래스 체계를 목표로 한다.
- RailSem19 라벨을 축약된 클래스 체계로 재매핑하는 규칙이 학습·평가·추론 전반에 공유된다.
- 경로 기본값에는 과거 Linux 학습 환경 흔적이 남아 있으므로, 실제 재학습 시에는 별도 경로
  정리가 필요할 수 있다.
"""
