"""DeepLabv3 기반 세그멘테이션 학습 진입 스크립트.

이 파일은 재구성 이후에도 과거 실험 경로를 보존하기 위해 유지된다. 학습 데이터
경로와 로그 경로는 스크립트 내부 상수로 유지되므로, 납품 검토 시에는
실험용 엔트리포인트로 보는 것이 적절하다.
"""

from src.common.dataloader_RailSem19 import CustomDataset
...
def get_image_4_wandb(path, input_size = [224,224]):
    """WandB 기록에 사용할 대표 이미지를 한 장 읽어 텐서로 만든다."""
...
def wandb_init(num_epochs, lr, batch_size, outputs, optimizer, scheduler):
    """학습 재현에 필요한 주요 하이퍼파라미터를 WandB에 기록한다."""
    wandb.init(
...
def create_model(output_channels=1):
    """DeepLabv3-ResNet50 분류 head를 현재 출력 클래스 수로 교체한다."""
...
def load_model(model_path):
    """기존 DeepLabv3 체크포인트를 학습 재개용으로 로드한다."""
...
def train(model, num_epochs, batch_size, image_size, optimizer, criterion):
    """Train/Valid 루프를 수행하며 손실과 segmentation 지표를 함께 누적한다."""
    start = time.time()
