"""SegFormer 기본 학습 진입 스크립트.

현재 저장소 기준에서 이 파일은 실험용 학습 엔트리포인트에 가깝다. 학습 데이터
경로, 로그 경로, epoch 수 등은 범용 CLI 인자가 아니라 스크립트 내부 상수로
고정되어 있으므로, 재현 실험 전에 경로와 하이퍼파라미터를 검토해야 한다.
"""

from src.common.dataloader_SegFormer import CustomDataset
from src.common.metrics_filtered_cls import compute_map_cls, compute_IoU
...
def get_image_4_wandb(path, input_size = [224,224]):
    """WandB 시각화용 대표 이미지를 한 장 읽어 텐서로 변환한다."""
...
def wandb_init(num_epochs, lr, batch_size, outputs, optimizer, scheduler, model):
    """현재 학습 설정을 WandB run config에 기록한다."""
    wandb.init(
...
def create_model(output_channels=1):
    """사전학습 SegFormer를 현재 출력 클래스 수에 맞춰 초기화한다."""
...
def load_model(model_path):
    """기존 체크포인트를 CPU 기준으로 로드한 뒤 현재 장치로 옮긴다."""
...
def train(model, num_epochs, batch_size, image_size, optimizer, criterion):
    """Train/Valid 분할을 순회하며 손실과 분류 지표를 함께 계산한다.

    이 스크립트는 WandB 기록, 주기적 체크포인트 저장, 최적 모델 보관을 모두
    하나의 루프에서 처리한다. 평가 지표는 `metrics_filtered_cls`의 클래스별
    mAP/IoU 계산 규칙에 의존한다.
    """
    start = time.time()
