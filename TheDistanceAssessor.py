...
# Import PyTorch model for debugging
from production_segformer_pytorch import load_pytorch_model

def load_model(model_path=None):
    """Load the PyTorch SegFormer model"""
    print("🚀 Loading PyTorch SegFormer model...")
...
def segment(model_seg, image_size, filename, PATH_jpgs, dataset_type, model_type, item=None):
        ...
        # Debug: Analyze segmentation results
        print(f"\n🔍 Segmentation Analysis for {filename}:")
        unique_classes = np.unique(id_map)
        print(f"📊 Detected classes: {unique_classes}")

        # Count pixels for each class
        total_pixels = id_map.size
        for cls in unique_classes:
            count = np.sum(id_map == cls)
            percentage = (count / total_pixels) * 100
            print(f"   Class {cls:2d}: {count:7d} pixels ({percentage:5.2f}%)")

        # Check potential rail classes (common rail classes in different systems)
        potential_rail_classes = [1, 2, 3, 4, 5, 9, 10, 12]
        rail_detected = []
        for rail_class in potential_rail_classes:
            if rail_class in unique_classes:
                count = np.sum(id_map == rail_class)
                if count > 100:  # Only consider significant detections
                    rail_detected.append(rail_class)

        if rail_detected:
            print(f"🚂 Potential rail classes detected: {rail_detected}")
        else:
            print("❌ No significant rail detections found")

        # Save segmentation mask for comparison
        ...
        # Segmentation mask
        im = ax2.imshow(id_map, cmap=cmap, vmin=0, vmax=19)
        ax2.set_title(f'Segmentation ({model_name} model)')
        ax2.axis('off')
