...
def create_tensorrt_engine(onnx_path, engine_path, model_name):
    """ONNX 모델을 대상 GPU 전용 TensorRT 엔진으로 변환한다.

    TensorRT 엔진은 GPU 아키텍처와 TensorRT 버전에 종속되므로,
    실제 배포 장비에서 다시 빌드하는 것이 가장 안전하다.
    """
    ...
    try:
        ...
        # 현재 파이프라인은 512x896, batch 1 입력을 사실상 고정으로 사용한다.
        # 다른 해상도나 배치를 허용하려면 메모리 한계와 성능을 다시 검증해야 한다.
...
def main():
    """세그멘테이션 ONNX 후보들을 순회하며 TensorRT 엔진 변환을 수행한다."""
