...
# -*- coding: utf-8 -*-
"""
RailSafeNet Final - Production Rail Detection System
=====================================================
Final production-ready implementation combining:
- Phase 4: Polynomial tracking for robust rail detection (113 FPS, 3.14px jitter)
- Phase 6: BEV direction detection (STRAIGHT/LEFT/RIGHT)

Features:
- Fullscreen display with mini BEV (Picture-in-Picture)
- Alert panel with danger status and direction
- YOLO object detection with hazard zone overlap
- Video and camera input modes

Usage:
    python3 videoAssessor_final.py --mode video              # Video mode
    python3 videoAssessor_final.py --mode camera --fullscreen # Camera mode
    python3 videoAssessor_final.py --calibrate               # BEV calibration
    python3 videoAssessor_final.py --calibrate-vp            # VP calibration (for hazard zones)
...
from videoAssessor import get_clues, find_edges, select_video_file, identify_ego_track, find_rail_sides
...
class FinalProcessor:
    ...
    def __init__(
        ...
        config_path: str = "config/rail_tracker_config.yaml",
        bev_config_path: str = "config/bev_config.yaml"
    ):
        ...
        self.tracker_config_path = config_path
    ...
    def _init_components(self, height: int, width: int):
        ...
        if self.bev_analyzer is None:
            self.bev_analyzer = BEVPathAnalyzer(config_path="config/bev_config.yaml")
...
def run_calibration(video_path: str, config_path: str = "config/bev_config.yaml"):
...
def run_vp_calibration(video_path: str, config_path: str = "config/rail_tracker_config.yaml"):
    """
    ...
    if vp:
        ...
        print("  python3 videoAssessor_final.py --mode video")
    else:
        print("\nCalibration cancelled")


def main():
    parser = argparse.ArgumentParser(description='RailSafeNet Final - Production System')
    parser.add_argument('--mode', choices=['video', 'camera'], default='video',
                       help='Input mode: video or camera')
    parser.add_argument('--video', type=str, default=None, help='Specific video path')
    parser.add_argument('--camera', type=int, default=0, help='Camera device ID')
    parser.add_argument('--config', type=str, default='config/rail_tracker_config.yaml',
                       help='Rail tracker config path')
    parser.add_argument('--bev-config', type=str, default='config/bev_config.yaml',
                       help='BEV config path')
    parser.add_argument('--fullscreen', action='store_true', help='Fullscreen display')
    parser.add_argument('--output', type=str, default=None,
                       help='Output video path (saves processed video without display)')
    parser.add_argument('--start-time', type=float, default=0,
                       help='Start time in seconds (skip frames before this)')
    parser.add_argument('--calibrate', action='store_true', help='Run BEV calibration')
    parser.add_argument('--calibrate-vp', action='store_true',
                       help='Run vanishing point calibration for hazard zones')
    args = parser.parse_args()

    # Use environment variable for Docker, fallback to relative path for native execution
    ...
    # VP (Vanishing Point) calibration mode
    ...
    print("=" * 70)
    print("RailSafeNet Final - Production Rail Detection System")
    print("=" * 70)
    ...
    if args.mode == 'video':
        if args.output:
            # Save mode (no display, just save to file)
